                    ...
                    for (si, s) in item.specifiers.iter().enumerate() {
                        ...
                        items.insert(
                            ...
                            ItemData {
                                ...
                                // Optimize by directly binding to the source. We preserve the
                                // import's `with` clause (e.g. `turbopack-transition`) so that
                                // re-emitting the import in the consuming fragment keeps the same
                                // resolution semantics. Dropping it caused transition imports to
                                // resolve incorrectly (undefined at evaluation) under tree-shaking.
                                binding_source: Some((
                                    *item.src.clone(),
                                    s.clone(),
                                    item.with.clone(),
