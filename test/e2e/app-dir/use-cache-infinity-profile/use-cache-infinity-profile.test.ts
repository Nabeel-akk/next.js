...
describe('use-cache-infinity-profile', () => {
  ...
  it('serves an inline Infinity cache life from a JSON-backed cache handler across requests', async () => {
    ...
    // The value is not part of the prerender, so the second request reads the
    // entry back from the cache handler, whose backing store has serialized
    // the cache life metadata as JSON. An infinite cache life must survive
    // that round trip; otherwise the entry is treated as immediately expired
    // and the value regenerates on every request.
