...
export type NavigationRequestAccumulation = {
   ...
   * as not instant (see finishSegmentCacheNode). The question is
   ...
   * matching the transition's instant default.
   */
...
// the flag attributes cache coverage, not paint time: whether those bytes
...
function createCacheNodeForSegment(
  ...
  // can mark it as not instant. See
  ...
  return finishSegmentCacheNode(
    accumulation,
    // Segments rendered from seed data (a server response already in hand)
    // belong to navigations that are either untracked or already marked at
    // the route level, so only the cache classification matters here. The
    // head is deliberately excluded: it always streams in non-blocking.
    seedRsc !== null || renderedFromCache,
    createCacheNode(rsc, prefetchRsc, head, prefetchHead, bfcacheId),
