...
- `event.instant` (commit only) - Whether the navigation was instant: at dispatch, the router had something cached to render for the whole destination — the route tree plus bytes for every fresh segment. "Something to render" is deliberately generous: a partial shell counts even when it is entirely a dynamic hole, so `instant` attributes cache coverage, not paint time. `false` when the navigation needed the network before it had anything to render: the route was not prefetched, a segment's prefetch was still pending or evicted, or the commit rode a refresh/retry-derived tree. Read it together with the start→commit latency: `false` marks navigations prefetching could have made faster, while a slow `true` commit is waiting on streaming runtime content or client-side rendering — costs prefetching cannot remove. In development nothing is prefetched, so most navigations accurately report `false`
